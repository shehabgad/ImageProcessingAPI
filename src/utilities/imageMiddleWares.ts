...
const imageExist = async (req: express.Request, res: express.Response, next: Function): Promise<void> => {
  const fileName = req.query.fileName as string
...
const cachedImageExist = async (req: express.Request, res: express.Response, next: Function): Promise<void> => {
  const fileName = req.query.fileName as string
  const width = req.query.width
  const height = req.query.height
  const result = await Image.imageExists("cachedImages", `${fileName}_${width}_${height}`)
  if (result == true) {
